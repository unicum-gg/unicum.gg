...
export async function generateMetadata({
  ...
  const { t } = await getTranslation("app/tournaments/page", locale);
  return constructMetadata({
    locale,
    title: t("title", { region: regionLabel }),
    description: t("description", { region: regionLabel }),
...
export default async function TournamentsPage({
  ...
  if (region === Region.EU) redirect(localizePath(ROUTES.TOURNAMENTS(Region.EU), locale));
  return renderTournaments(region, locale);
}

/** The catalogue itself, shared by the regional route and the EU shortcut. */
export async function renderTournaments(region: Region, locale: string) {
  const { t } = await getTranslation("app/tournaments/page", locale);
        ...
        <PanelContent className="px-4 py-12 text-center">
          ...
          <h1 className="font-heading text-4xl font-bold tracking-tight md:text-5xl">
            <Interpolate
              ...
              values={{
                tournaments: (
                  <span className="text-brand">{tGame("features.tournaments")}</span>
                ),
              }}
            />
          </h1>
          <p className="mx-auto mt-4 max-w-2xl text-fd-muted-foreground">
            {live > 0
              ? t(live === 1 ? "live-one" : "live-many", {
                  count: live,
                  region: REGION_LABEL[region],
                })
              : t("no-live", { region: REGION_LABEL[region] })}{" "}
            {t("what-you-find")}
          </p>
        </PanelContent>
      </Panel>
