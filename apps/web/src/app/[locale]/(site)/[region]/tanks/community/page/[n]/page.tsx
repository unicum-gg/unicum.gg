...
export async function generateMetadata({
  ...
  return paginatedMetadata(await tankCommunityMetadata(region, locale), { page, locale });
}
...
export default async function Page({
  ...
  if (region === Region.EU) {
    redirect(
      `${localizePath(ROUTES.TANKS_COMMUNITY(Region.EU), locale)}?${PAGINATION.PARAM}=${page}`,
    );
  }
  return (
    <TankCommunityView locale={locale} region={region} page={page} />
  );
}
