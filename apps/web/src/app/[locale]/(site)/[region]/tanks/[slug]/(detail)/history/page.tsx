...
import { loadTankHistory } from "@/app/[locale]/(site)/[region]/tanks/[slug]/detail";
import {
  loadTankTab,
  tankMetadata,
...
export async function generateMetadata({
  ...
  return tankMetadata(region, slug, TankDetailTab.History, locale);
}
...
export default async function TankHistoryPage({
  ...
  const [detail, history] = await Promise.all([
    loadTankTab(region, slug, TankDetailTab.History, locale),
    loadTankHistory(region, slug),
  ]);
  return (
    <TankChangesHistory locale={locale}
