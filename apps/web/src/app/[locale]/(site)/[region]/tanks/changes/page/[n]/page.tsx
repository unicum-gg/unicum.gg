...
export async function generateMetadata({
  ...
  return paginatedMetadata(await tankChangesMetadata(region, locale), { page, locale });
}
...
export default async function Page({
  ...
  if (region === Region.EU) {
    redirect(
      `${localizePath(ROUTES.TANKS_CHANGES(Region.EU), locale)}?${PAGINATION.PARAM}=${page}`,
    );
  }
  return (
    <TankChangesView locale={locale} region={region} page={page} />
  );
}
