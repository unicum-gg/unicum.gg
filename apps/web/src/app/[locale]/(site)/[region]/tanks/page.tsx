...
export async function tanksIndexMetadata(
  ...
  const { t } = await getTranslation("components/tanks/list/copy", locale);
  const { title, description } = tankTabCopy(tab, region.toUpperCase(), t);
...
export async function generateMetadata({
  ...
  return tanksIndexMetadata(region, TankTab.Performances, locale);
}
...
export default async function TanksIndexPage({
  ...
  return renderTanksIndex(region, locale);
}
...
export async function renderTanksIndex(
  ...
  if (activeTab === TankTab.Videos) {
    ...
    return (
      <TanksIndexShell
        ...
        locale={locale}
        tab={activeTab}
        count={numberFormat(locale, INT_FORMAT).format(list.length)}
      >
        <TanksVideosTab
          region={region}
          battles={list}
  ...
  return (
    <TanksIndexShell
      ...
      locale={locale}
      tab={activeTab}
      count={numberFormat(locale, INT_FORMAT).format(items.length)}
    >
      <TanksIndex
        tanks={items}
        region={region}
...
async function TanksIndexShell({
  region,
  locale,
  tab,
  count,
  ...
  const { t } = await getTranslation("components/tanks/list/copy", locale);
  const copy = tankTabCopy(tab, REGION_LABEL[region], t);
  return (
