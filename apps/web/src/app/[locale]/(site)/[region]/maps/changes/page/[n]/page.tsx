...
export async function generateMetadata({
  ...
  return paginatedMetadata(await mapChangesMetadata(region, locale), { page, locale });
}
...
export default async function Page({
  ...
  if (region === Region.EU) {
    redirect(
      `${localizePath(ROUTES.MAPS_CHANGES(Region.EU), locale)}?${PAGINATION.PARAM}=${page}`,
    );
  }
  return (
    <MapChangesView locale={locale} region={region} page={page} />
  );
}
