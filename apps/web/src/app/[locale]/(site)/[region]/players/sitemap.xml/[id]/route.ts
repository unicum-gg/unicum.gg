...
export async function GET(
  ...
  const entries = rows.map((row) =>
    createLocalizedSitemapEntry(ROUTES.PLAYER(region, row.nickname), {
  ...
  return new Response(generateSitemapXml(entries), {
    headers: {
      ...
      // this runs now: paging with OFFSET into two million rows and rendering
      // ~19 MB is not something a crawler hit should pay for, and a player who
