...
export async function generateMetadata({
  ...
  return paginatedMetadata(await onslaughtMetadata(region, locale), { page, locale });
}
...
export default async function Page({
  ...
  if (region === Region.EU) {
    redirect(
      `${localizePath(ROUTES.PLAYERS_ONSLAUGHT(Region.EU), locale)}?${PAGINATION.PARAM}=${page}`,
    );
  }
  return (
    <OnslaughtView locale={locale} region={region} page={page} />
  );
}
