...
export async function generateMetadata({
  ...
  return paginatedMetadata(await playersLandingMetadata(region, locale), { page, locale });
}
...
export default async function Page({
  ...
  // EU reads at the region-less address, its page included, like the section
  // itself does.
  if (region === Region.EU) {
    redirect(
      `${localizePath(ROUTES.PLAYERS(Region.EU), locale)}?${PAGINATION.PARAM}=${page}`,
    );
  }
  return (
    <PlayersLandingView locale={locale} region={region} language={null} page={page} />
  );
}
