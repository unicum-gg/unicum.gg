...
export async function generateMetadata({
  ...
  return paginatedMetadata(await steelHunterMetadata(region, locale), { page, locale });
}
...
export default async function Page({
  ...
  if (region === Region.EU) {
    redirect(
      `${localizePath(ROUTES.PLAYERS_STEEL_HUNTER(Region.EU), locale)}?${PAGINATION.PARAM}=${page}`,
    );
  }
  return (
    <SteelHunterView locale={locale} region={region} page={page} />
  );
}
