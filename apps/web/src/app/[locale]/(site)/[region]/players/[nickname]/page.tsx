...
function viewCopy(
  ...
  // Which block of `app/players/detail/page` this view reads, and the values it
  // needs. The mode name is the game's own, looked up by the caller.
  const values = { name, region: regionLabel, battles, winrate, rating, medals, mode };
  const block =
    view.section === PlayerSection.Tanks
      ? "tanks"
      : view.section === PlayerSection.Achievements
        ? "achievements"
        : view.section === PlayerSection.Tournaments
          ? "tournaments"
          : view.section === PlayerSection.Value
            ? "value"
            : view.mode !== PlayerMode.Overall
              ? "mode"
              : "overview";
  return {
...
export async function playerMetadata(
  ...
  const { t } = await getTranslation("app/players/detail/page", locale);
  ...
  if (result && "locked" in result) {
    ...
    locale,
      title: t("locked.title", { name: result.nickname, region: regionLabel }),
  ...
  if (!detail || detail.current.battles === 0) {
    return constructMetadata({
    locale,
      title: t("empty.title", { name: displayName, region: regionLabel }),
  ...
  const copy = viewCopy(
    ...
    displayName,
    regionLabel,
    numberFormat(locale, INT_FORMAT).format(current.battles),
    numberFormat(locale, PCT_FORMAT).format((current.wins / current.battles) * 100),
    numberFormat(locale, INT_FORMAT).format(current.wtr ?? current.globalRating),
    numberFormat(locale, INT_FORMAT).format(detail.achievementCount),
    t,
    tGame(`player-modes.${view.mode}`),
  );
  return constructMetadata({
    locale,
    title: copy.title,
...
export async function generateMetadata({
  ...
  params: Promise<{ locale: string; region: string; nickname: string }>;
}): Promise<Metadata> {
  const { locale, region, nickname } = await params;
  return playerMetadata(region, nickname, DEFAULT_PLAYER_VIEW, locale);
}
...
async function PlayerProfileServer({
  ...
  decoded: string;
  view: PlayerView;
  /** Carried down only to keep the canonical-nickname redirect in the
   * reader's language: a bare path would leave the proxy to guess it back. */
  locale: string;
  tankSlug?: string;
}) {
  ...
  const initialAchievements: PlayerAchievements | null =
    section === PlayerSection.Achievements
      ? ((await unicum
          .region(region)
          .players(decoded)
          .achievements(locale)) as unknown as PlayerAchievements)
      : null;
  ...
  const playerDescription =
    current.battles > 0
      ? `${displayName} (${regionLabel}) World of Tanks player stats: ${numberFormat(locale, INT_FORMAT).format(current.battles)} battles, ${numberFormat(locale, PCT_FORMAT).format(winrate)}% winrate, WN8 and WNX ratings, tank-by-tank breakdown and clan history.`
