...
import { toTankClient } from "@unicum.gg/shared";
...
export async function GET(
  ...
  return measured("GET /api/{region}/tanks/{slug}/detail", async () => {
    ...
    // Anything but `ct` is live, including a typo: the characteristics of a
    // vehicle are not something to 400 over a query param on.
    const client = toTankClient(new URL(req.url).searchParams.get("client"));
    ...
    const cached = await getCachedTankDetailJson(region, decoded, client);
    ...
    const payload = await traced("assembleTankDetail", () =>
      assembleTankDetail(region, decoded, client),
    );
    if (!payload) {
    ...
    void setCachedTankDetailJson(region, decoded, json, client);
