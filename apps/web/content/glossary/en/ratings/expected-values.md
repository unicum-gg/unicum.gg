...
Every per-vehicle rating needs a baseline. Expected values are that baseline, one row per vehicle giving the average damage, frags, spotting and defence the server produces in it. A player's own numbers are divided by these to get the ratios the rating formula runs on.

The tables are maintained by the community, not by Wargaming, and are recomputed periodically from a large sample of accounts. Between two releases the vehicle roster keeps moving: new premiums and reward vehicles have no row yet, so a rating either skips them or falls back to the average of comparable vehicles at the same tier and class.
