...
import { CLAN_TABS, ClanTab, clanTabHref } from "./tabs";
...
export function ClanTabsNav({
  ...
  function handleClick(event: MouseEvent<HTMLAnchorElement>, tab: ClanTab) {
    if (
      event.button !== 0 ||
      event.metaKey ||
      event.ctrlKey ||
  ...
  return (
    <nav className="flex items-center overflow-x-auto text-sm">
      {CLAN_TABS.map((t) => (
        <a
          key={t.id}
          href={clanTabHref(basePath, t.id)}
