...
- **`apps/bot/`** (`@unicum.gg/bot`) — a standalone Discord bot on the `dixt` framework (`tsx`, no build; sets `__dbContext = "background"` like the worker). Slash commands live in `src/plugins/wot/commands/` (`/player`, `/clan`); both reply ephemerally with a "Share to channel" button and render stats as aligned code-block tables mirroring the site's tables (`lib/{table,stats-lines,clan-lines}.ts`). **Command data is fetched through `@unicum.gg/sdk`** (`lib/sdk.ts` — dogfooding the public API; the `/{region}/players/{nickname}` and `/{region}/clans/{tag}` endpoints already handle the cold-DB case server-side, so the bot no longer duplicates the resolve-on-WG + record-snapshot logic). It still uses `@unicum.gg/core` for the two things the SDK can't do: **autocomplete** via the instant local DB search (`searchPlayersLocal`/`searchClansLocal`, not the slow WG remote search — Discord's 3s autocomplete window) and `APP_IDENTITY` for the profile/OG URLs. Deployed as its own Coolify service (env `DIXT_*` + `DATABASE_URL`/`REDIS_URL`, plus **`UNICUM_API_URL`** pointed at the internal API container so the SDK's data calls stay on the Coolify private network; unset falls back to the public app URL). Local dev: `apps/bot/.env.local` overrides `NEXT_PUBLIC_APP_URL=https://unicum.gg` so Discord can server-side-fetch the OG images (it can't reach localhost).
...
**Package layering (target).** Dependencies flow one way, no cycles: `@unicum.gg/wargaming` → `@unicum.gg/core` → { `apps/worker`, the web API route handlers, `@unicum.gg/sdk` }, and `@unicum.gg/sdk` → the web **front** (components/pages). The front is meant to reach the API through the SDK rather than importing core directly, so hand-written `fetch`/SWR fetchers get replaced by the fluent client. Wiring the whole front onto the SDK is a large, in-progress refactor.
