...
- **`apps/bot/`** (`@unicum.gg/bot`) — a standalone Discord bot on the `dixt` framework (`tsx`, no build), reusing `@unicum.gg/core` for all data (sets `__dbContext = "background"` like the worker). Slash commands live in `src/plugins/wot/commands/` (`/player`, `/clan`); both reply ephemerally with a "Share to channel" button, autocomplete via the instant local DB search (`searchPlayersLocal`/`searchClansLocal`, not the slow WG remote search), and render stats as aligned code-block tables mirroring the site's tables (`lib/{table,stats-lines,clan-lines}.ts`). Deployed as its own Coolify service (env `DIXT_*` + `DATABASE_URL`/`REDIS_URL`). Local dev: `apps/bot/.env.local` overrides `NEXT_PUBLIC_APP_URL=https://unicum.gg` so Discord can server-side-fetch the OG images (it can't reach localhost).
