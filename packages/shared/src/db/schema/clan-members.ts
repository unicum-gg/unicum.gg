...
export function makeClanMembersTable(region: string) {
  return pgTable(
    ...
    (t) => [primaryKey({ columns: [t.clanId, t.accountId] })],
  );
}

export type ClanMembersTable = ReturnType<typeof makeClanMembersTable>;
export type ClanMember = ClanMembersTable["$inferSelect"];
export type NewClanMember = ClanMembersTable["$inferInsert"];

export const clanMembersByRegion: Record<Region, ClanMembersTable> = {
  [Region.EU]: makeClanMembersTable(Region.EU),
  [Region.NA]: makeClanMembersTable(Region.NA),
  [Region.ASIA]: makeClanMembersTable(Region.ASIA),
};
